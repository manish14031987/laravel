...
class ReportController extends Controller {
  ...
  public function timesheetpdf($reportProject_to = null, $reportProject_from = null) {
    ...
    $query->select(DB::raw('project.*', 'project_phase.phase_Id', 'taskassign.id as task_id', 'users.name as first_name', 'users.lname as last_name', 'budget_original.overall as budget_org_overall', 'budget_return.overall as budget_return_overall', 'budget_supplement.overall as budget_supplement_overall', 'sum(project_gr_cost.debit)-sum(project_gr_cost.credit) as actuall_cost'));
  ...
  public function projectportfoliopdf($reportProject_to = null, $reportProject_from = null, $reportbucket_id = null, $reportportfolio_id = null) {
    ...
    $query->select(DB::raw('project.*', 'project_phase.phase_Id', 'taskassign.id as task_id', 'users.name as first_name', 'users.lname as last_name', 'budget_original.overall as budget_org_overall', 'budget_return.overall as budget_return_overall', 'budget_supplement.overall as budget_supplement_overall', 'sum(project_gr_cost.debit)-sum(project_gr_cost.credit) as actuall_cost', 'buckets.name as bucket_name', 'buckets.bucket_id', 'portfolio.name as portfolio_name'));
    $query->leftJoin('project_phase', 'project_phase.project_id', '=', 'project.id');
    $query->leftJoin('budget_original', 'budget_original.project_id', '=', 'project.project_Id');
    $query->leftJoin('budget_return', 'budget_return.project_id', '=', 'project.project_Id');
    $query->leftJoin('budget_supplement', 'budget_supplement.project_id', '=', 'project.project_Id');
  ...
  public function getProjectData(){
        $pid = Input::get('id');
        $project_data = ProjectHelpers::getProjectData($pid);
        return response()->json(array('data' => $project_data[0]));
    }
